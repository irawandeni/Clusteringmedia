...
$mysql=new mysql("localhost","root","","db_clustering");
//Cek Koneksi
...
$jumlahmahasiswa=caridata($mysql,"select count(*) from tb_mahasiswa");
...
while ($status=='false'){
	...
	$centro3[$loop][0]=caridata($mysql,"select avg(tanggungan) from tb_mahasiswa where set_sementara='C3'");
	$centro3[$loop][1]=caridata($mysql,"select avg(k_pekerjaan) from tb_mahasiswa where set_sementara='C3'");
	$centro3[$loop][2]=caridata($mysql,"select avg(k_penghasilan) from tb_mahasiswa where set_sementara='C3'");
