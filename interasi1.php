...
?> 
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Hasil Clustering Interasi 1</h1>
                        <h1 class="page-subhead-line"> Hasil Interasi 1 ...</h1>
...
if($_GET[act]=="lihat"){
  ...
  $queryutama = mysql_query("SELECT  *  FROM kasus where idk='$_GET[id]'"); 
    ...
    while ($data = mysql_fetch_array($queryutama)) {
		 $c1=sqrt((pow(($data[kharga]-22.18009768),2))+(pow(($data[kmenu]-19.17826618),2))+(pow(($data[kfasilitas]-19.20940171),2))+(pow(($data[kjam]-19.2026862),2)));
		 $c2=sqrt((pow(($data[kharga]-25.19912281),2))+(pow(($data[kmenu]-21.42207602),2))+(pow(($data[kfasilitas]-21.45716374),2))+(pow(($data[kjam]-21.68815789),2)));
		 $c3=sqrt((pow(($data[kharga]-27.8),2))+(pow(($data[kmenu]-23.32),2))+(pow(($data[kfasilitas]-23.24),2))+(pow(($data[kjam]-24),2)));
		 
		 
		 $min=min($c1,$c2,$c3);
		 if($min==$c1){
			$hasil="(C1)Cluster Buruk";
		 }elseif($min==$c2){
			$hasil="(C2) Cluster Cukup";
		}
		 elseif($min==$c3){
			$hasil="(C3) Cluster Baik";
		 }
		echo"<h3>Perhitungan ID Kasus $_GET[id]</h3>";
		echo"<p><b>Hitung jarak ID Kasus $_GET[id] terhadap cluster Pertama:</b></p>";
		echo"<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C$_GET[id]1 = √((($data[kharga]-22.18009768)^2)+(($data[kmenu]-19.17826618)^2)+(($data[kfasilitas]-19.20940171)^2)+(($data[kjam]-19.2026862)^2)) = $c1 </p>";
		echo"<p><b>Hitung jarak ID Kasus $_GET[id] terhadap cluster Kedua:</b></p>";
		echo"<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C$_GET[id]2 = √((($data[kharga]-25.19912281)^2)+(($data[kmenu]-21.42207602)^2)+(($data[kfasilitas]-21.45716374)^2)+(($data[kjam]-21.68815789)^2)) = $c2</p>";
...
echo"<p><a href='chart.php' target='_blank'  class='btn btn-primary'> Hasil </a></p>";
