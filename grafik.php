...
<body>
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ["Jatimulyo", "Kalirancang", "Kalijirek", "Candiwulan"],
                datasets: [{
                    label: '',
                    data: [
                    <?php 
                    $Jatimulyo = mysql_query("select * from siswa where alamat='Jatimulyo");
                    echo mysql_num_rows($Jatimulyo);
                    ?>, 
                    <?php 
                    $Kalirancang = mysql_query("select * from siswa where alamat='Kalirancang'");
                    echo mysql_num_rows($Kalirancang);
                    ?>, 
                    <?php 
                    $Kalijirek = mysql_query("select * from tb_siswa where alamat='Kalijirek'");
                    echo mysql_num_rows($Kalijirek);
                    ?>, 
                    <?php 
                    $Candiwulan = mysql_query("select * from siswa where jurusan='Candiwulan'");
                    echo mysql_num_rows($Candiwulan);
                    ?>
                    ],
                    backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
